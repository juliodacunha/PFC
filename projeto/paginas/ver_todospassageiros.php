...
<h3 class="register-heading my-5 mx-auto" style="font-family: 'CustomFont'; font-weight:normal; font-style:normal; font-size: 30PX; text-align: center;">Data para ver TODOS passageiros</h3>
...
if($tipo_usuario == 2){
    ...
    if(isset($_GET['mes'])){
      ...
      and tipuser_tip_user = tip_user and descricao = 'passageiro' and id_usuario = id_usuario_id and id_passageiro = id_passageiro_id") or die ($link->error);

      $consulta2 = $link->query("SELECT 
      id_usuario, nome, sobrenome, telefone, email, sexo, bairro, rua, numero, complemento, id_passageiro, id_motorista_id, telefone 
      from usuarios, passageiros, enderecos, corridas
      where id_usuario = id_usuario_id and id_passageiro = id_passageiro_id and id_usuario = usuario_id_usuario and data_corrida = '$data' and horario_ida= 'Nao vai'") or die ($link->error);
      ...
      id_usuario, nome, sobrenome, telefone, email, sexo, bairro, rua, numero, complemento, id_passageiro, id_motorista_id, telefone
      from usuarios, passageiros, enderecos, corridas
      where id_usuario = id_usuario_id and id_passageiro = id_passageiro_id and id_usuario = usuario_id_usuario and data_corrida = '$data' and horario_volta= 'Volta 12h'") or die ($link->error);

      $consulta5 = $link->query("SELECT id_usuario, nome, sobrenome, telefone, email, sexo, bairro, rua, numero, complemento, id_passageiro, id_motorista_id, telefone
      from usuarios, passageiros, enderecos, corridas
      where id_usuario = id_usuario_id and id_passageiro = id_passageiro_id and id_usuario = usuario_id_usuario and data_corrida = '$data' and horario_volta = 'Nao volta'") or die ($link->error);
