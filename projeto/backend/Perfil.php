...
if(isset($_POST['editar'])){
    if(isset($cnh)){
        $nome = mysqli_real_escape_string($link, $_REQUEST['novonome']);
        $sobrenome = mysqli_real_escape_string($link, $_REQUEST['novosobrenome']);
        $senha = mysqli_real_escape_string($link, $_REQUEST['novosenha']); 
        $newsenha = md5($senha);
        $rg = mysqli_real_escape_string($link, $_REQUEST['novorg']);
        $cpf = mysqli_real_escape_string($link, $_REQUEST['novocpf']);
        //$email = mysqli_real_escape_string($link, $_REQUEST['novoemail']);
        $telefone = mysqli_real_escape_string($link, $_REQUEST['novotelefone']);
        //$sexo = mysqli_real_escape_string($link, $_REQUEST['sexo']);
        $cnh = mysqli_real_escape_string($link, $_REQUEST['novocnh']);
        //echo $nome;
        if($senha != '<!>&&<!>'){
            $sql2 = "UPDATE usuarios SET senha='$newsenha' where id_usuario = '$id_usuario'";
        }
        ...
        $cpf = mysqli_real_escape_string($link, $_REQUEST['novocpf']);
        $telefone = mysqli_real_escape_string($link, $_REQUEST['novotelefone']);
        $curso = mysqli_real_escape_string($link, $_REQUEST['novocurso']);
        $turma = mysqli_real_escape_string($link, $_REQUEST['novoturma']);
        $matricula = mysqli_real_escape_string($link, $_REQUEST['novomatricula']);
        $cep = mysqli_real_escape_string($link, $_REQUEST['novocep']);
        $rua = mysqli_real_escape_string($link, $_REQUEST['novorua']);
        $numero = mysqli_real_escape_string($link, $_REQUEST['novonumero']);
        $bairro = mysqli_real_escape_string($link, $_REQUEST['novobairro']);
        $cidade = mysqli_real_escape_string($link, $_REQUEST['novocidade']);
        $estado = mysqli_real_escape_string($link, $_REQUEST['novoestado']);
        $complemento = mysqli_real_escape_string($link, $_REQUEST['novocomplemento']);
        if($senha != '<!>&&<!>'){
            $sql2 = "UPDATE usuarios SET senha='$newsenha' where id_usuario = '$id_usuario'";
        }
    ...
    if (mysqli_query($link, $sql)) {
        ...
        echo "Erro ao editar o perfil: " . mysqli_error($link);
    }
}else{}
